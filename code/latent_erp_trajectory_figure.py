#!/usr/bin/env python3
"""Figures for ERP latent trajectory analysis."""
...
def save_latent_erp_trajectory_plot(output_dir, figures_dir):
    d = require_csv(output_dir / "latent_erp_trajectory_points.csv")
    ...
    ax.set_title("ERP Latent Trajectories")
    ax.set_xlabel("latent 1")
    ax.set_ylabel("latent 2")
    ax.legend(frameon=False, loc="best")
    setup_axis(ax)
    fig.tight_layout()
    path = figures_dir / "latent_erp_trajectories.png"
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[latent ERP figure] wrote {path}", flush=True)
...
def save_latent_erp_matrix_plot(output_dir, figures_dir):
    d = require_csv(output_dir / "latent_erp_trajectory_group_distances.csv")
    ...
    ax.set_title("ERP Latent Day Distances")
    ...
    fig.tight_layout()
    path = figures_dir / "latent_erp_day_geometry_matrix.png"
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[latent ERP figure] wrote {path}", flush=True)


def save_latent_erp_model_evidence(output_dir, figures_dir):
    d = require_csv(output_dir / "latent_erp_trajectory_model_subject.csv")
    ...
    ax.set_title("ERP Latent 5x5 Model Evidence")
    ...
    setup_axis(ax)
    fig.tight_layout()
    path = figures_dir / "latent_erp_model_evidence.png"
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[latent ERP figure] wrote {path}", flush=True)


def save_latent_erp_figures(output_dir=OUTPUT_DIR, figures_dir=FIGURES_DIR):
    output_dir = Path(output_dir)
    figures_dir = Path(figures_dir)
    figures_dir.mkdir(parents=True, exist_ok=True)
    save_latent_erp_trajectory_plot(output_dir, figures_dir)
    save_latent_erp_matrix_plot(output_dir, figures_dir)
    save_latent_erp_model_evidence(output_dir, figures_dir)


if __name__ == "__main__":
    save_latent_erp_figures()
