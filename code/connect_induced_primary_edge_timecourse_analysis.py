#!/usr/bin/env python3
"""Total-vs-induced primary dwPLI strict sensor-edge time courses."""
...
N_JOBS = 2
OUTPUT_PREFIX = "connect_induced_primary"
...
def make_tasks(subject_ids=None, max_sessions=None):
    ...
    if not sessions:
        raise ValueError("No sessions selected for induced primary connectivity")
...
def run_connect_induced_primary_edge_timecourse(
    ...
    if used == 0 or info_subset is None or not edge_path.exists():
        raise ValueError("No induced primary edge rows were computed")
        ...
        json.dumps(
            {
                ...
                "bands": list(PRIMARY_BANDS.keys()),
                "signal_estimates": ["total", "induced"],
                "elapsed_sec": float(time.time() - t0),
            },
            indent=2,
        )
    )
    print(f"[connect induced primary edges] wrote {edge_path}", flush=True)
    return {
...
if __name__ == "__main__":
    run_connect_induced_primary_edge_timecourse()
