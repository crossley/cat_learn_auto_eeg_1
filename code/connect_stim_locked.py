#!/usr/bin/env python3
"""Stimulus-locked visual-motor functional connectivity (abs ImCoh)."""
...
def process_stim_locked_session(task):
    ...
    tmin = float(task["tmin"])
    tmax = float(task["tmax"])

    try:
        epochs = mne.read_epochs(epo_path, preload=False, verbose="ERROR")
        epochs, _ = align_behaviour_to_epochs(beh_df, epochs, event_names=("Stim/A", "Stim/B"))
        ...
        start_times = np.arange(tmin, tmax - window_sec + 1e-12, step_sec)
...
def run_stim_locked_connectivity_analysis(
    ...
    t0 = time.time()
    progress_json = output_dir / "connect_stim_locked_progress.json"
    ...
    print(
        f"[connect_stim_locked] Running on {len(tasks)} sessions (n_workers={n_workers})...",
        flush=True,
    )
    results = parallel_collect(process_stim_locked_session, tasks, n_workers)
    ...
    if skipped:
        pd.DataFrame(skipped).to_csv(
            output_dir / "connect_stim_locked_qc_skipped.csv", index=False
        )

    d = pd.DataFrame(all_rows)
    output_csv = output_dir / "connectivity_stim_locked_subject_day.csv"
    d.to_csv(output_csv, index=False)

    if d.empty:
        print("[connect_stim_locked] No data to plot.", flush=True)
    ...
    fig.suptitle("Stim-locked abs(ImCoh), peak-normalized", y=1.02)
    fig.tight_layout()
    fig_path = figures_dir / "connectivity_stim_locked.png"
...
if __name__ == "__main__":
    run_stim_locked_connectivity_analysis()
