#!/usr/bin/env python3
"""Strict-ROI stimulus category MVPA split by bound-distance difficulty."""
...
def run_mvpa_stim_locked_cat_difficulty_roi(
    ...
    session_csv = output_dir / "mvpa_stim_locked_cat_difficulty_roi_session_timecourse.csv"
    subject_day_csv = output_dir / "mvpa_stim_locked_cat_difficulty_roi_subject_day_timecourse.csv"
    day_mean_csv = output_dir / "mvpa_stim_locked_cat_difficulty_roi_day_means_timecourse.csv"
    qc_csv = output_dir / "mvpa_stim_locked_cat_difficulty_roi_qc_log.csv"
    progress_json = output_dir / "mvpa_stim_locked_cat_difficulty_roi_progress.json"
    t0 = time.time()
    ...
    def handle_result(result, done):
        ...
        if (done % 5) == 0:
            print(
                f"[MVPA difficulty ROI] complete {done}/{len(tasks)} sessions "
                f"(elapsed {(time.time() - t0) / 60.0:.1f} min)",
                flush=True,
            )

    print(
        f"[MVPA difficulty ROI] Running {len(tasks)} sessions "
    ...
    if session_df.empty:
        ...
        raise RuntimeError("Difficulty ROI MVPA produced no valid session rows")

    subject_day_df = (
        session_df.groupby(["difficulty", "roi", "subject", "day", "time_sec"], as_index=False)
        .agg(auc=("auc", "mean"), n_channels=("n_channels", "max"))
        .sort_values(["difficulty", "roi", "subject", "day", "time_sec"])
    )
    day_mean_df = (
        subject_day_df.groupby(["difficulty", "roi", "day", "time_sec"], as_index=False)
        .agg(
            auc_mean=("auc", "mean"),
            auc_sem=("auc", sem),
        ...
        .sort_values(["difficulty", "roi", "day", "time_sec"])
    )
    session_df.to_csv(session_csv, index=False)
    ...
    print(f"[MVPA difficulty ROI] wrote {day_mean_csv}", flush=True)
    return {
        "session": session_csv,
...
if __name__ == "__main__":
    run_mvpa_stim_locked_cat_difficulty_roi()
