...
def plot_edge_time_carpet(day_data, pair_idx, lock_name, band_name, figures_dir):
    ...
    n_edges = len(pair_idx)
    fig, axes = plt.subplots(len(rows), 1, figsize=(10, 2.1 * len(rows)), sharex=True)
    if len(rows) == 1:
        axes = [axes]
    vmax = max(float(np.nanmax(np.abs(d))) for _, _, d in rows if np.isfinite(d).any())
    vmax = max(vmax, 1e-12)
    for ax, (day, times, d) in zip(axes, rows):
        im = ax.imshow(
            ...
            origin="lower",
            aspect="auto",
            extent=[float(times.min()), float(times.max()), 0, n_edges],
            cmap="RdBu_r",
            vmin=-vmax,
            vmax=vmax,
        )
...
def save_fig_sensorwide_connectivity(
    ...
    d_edges = pd.read_csv(edges_path)
    if d_edges.empty:
        return {"figure_paths": []}
    ...
    for lock_name in ("stim", "feedback"):
        for band_name in bands.keys():
            day_data = {}
            d_lb = d_edges[
                (d_edges["lock_type"] == lock_name) & (d_edges["band"] == band_name)
            ]
            for day in all_days:
            ...
            fig_path = plot_edge_time_carpet(
