#!/usr/bin/env python3
"""25-block connectivity model evidence for multi-measure ROI edges."""
    ...
    try:
        for block, idx in beh.groupby("block").groups.items():
            ...
            for key, group in rows.groupby(["measure", "band", "lock_time"]):
                ...
                vectors[
                    (str(measure), str(band), subject, int(block), float(lock_time))
                ] = vals
...
def make_tasks(subject_ids=None, max_sessions=None):
    ...
    if not sessions:
        raise ValueError("No sessions selected for multi-measure block model")
...
def score_vectors(vectors):
    ...
    for key, vec in vectors.items():
        measure, band, subject, block, time_sec = key
        by_group.setdefault((measure, band), {}).setdefault(time_sec, {}).setdefault(
            subject, {}
        )[block] = vec
    rows = []
    for measure, band in sorted(by_group):
        by_time = by_group[(measure, band)]
        ...
        for time_i, time_sec in enumerate(times, start=1):
            ...
            if (time_i % 10) == 0:
                print(
                    "[connect multimeasure block] "
                    f"{measure}/{band} times {time_i}/{len(times)}",
                    flush=True,
                )
    if not rows:
        raise ValueError("No multi-measure block-model rows were produced")
    return rows


def run_connect_multimeasure_block_model_timecourse(
    ...
    if not vectors:
        raise ValueError("No multi-measure block vectors were computed")
    ...
    write_grouped_scores(rows, ["measure", "band"], subject_path, summary_path)
    print(f"[connect multimeasure block] wrote {subject_path}", flush=True)
    print(f"[connect multimeasure block] wrote {summary_path}", flush=True)
    return {"subject": subject_path, "summary": summary_path}


if __name__ == "__main__":
    run_connect_multimeasure_block_model_timecourse()
